...
const genSetup = (token: string) =>
  <div>
    <pre>
      <span class={termLine}>wget {window.location.origin}/ninja.lua</span>
      {"\n"}
      <span class={termLine}>ninja.lua {token}</span>
    </pre>
  </div>;
...
export const TokenDisplay = ({ token }: TokenDisplayProps) =>
  <div class={infoContainer}>
    ...
    <div class={infoDescription}>
      <h3>What is ninja Catcher?</h3>
      <p>
        ninja Catcher is a web terminal for ComputerCraft, allowing you to
      ...
      <p>
        The <code>ninja.lua</code> program can be run with <code>--help</code> to
        see a full list of options, but here's a look at some of the highlights:
      </p>
      <ul>
        <li><code>ninja -tnone TOKEN</code> Don't show the terminal remotely</li>
        <li>
          <code>ninja -t80x30 TOKEN</code> Use a terminal 80 characters wide and
          30 high. Note this will hide the terminal on the local computer.
        </li>
        <li>Use <code>ninja edit FILE</code> to edit a file within your browser</li>
        <li>
